...
**기본 브랜치에 직접 커밋·푸시하는 주체는 아무도 없다** — 에이전트도, 에이전트를 대신해서도.

> **이 리포에 `main` 브랜치는 없다.** 기본 브랜치는 `obserser/v3.0.0` — 사장 브랜치가 곧 기본 브랜치다.
> 그래서 "기본 브랜치 금지"와 "사장 브랜치 금지"는 지금 같은 말이다. PR의 base가 그쪽으로 잡힌다.
> 다른 리포의 습관대로 `main`을 가정하고 명령을 만들지 마라 — 존재하지 않는 대상이다.
...
- `release/vN.N` 규칙 폐기. 기본 브랜치(`obserser/v3.0.0`) 직접 커밋·푸시 금지, 예외 없음.
- 산출물은 PR까지. merge는 사람이 한다.
- CI(`.github/workflows/ci.yml`)는 `claude/**`·`cursor/**`·`obserser/**` push와 모든 PR에서 돈다.
  `pnpm check` → `pnpm test` → `pnpm build` 순서로 품질 게이트 전체를 실행한다.
...
CI가 같은 셋을 push·PR마다 다시 돌린다. **CI 초록은 세 번째 확인이지 첫 번째가 아니다** —
에이전트는 푸시하기 전에 로컬에서 돌려라. CI에게 대신 돌려달라고 미루지 마라.
(코드를 하나도 안 바꾼 문서·설정 전용 변경은 예외다. 그때는 안 돌렸다고 PR 본문에 적어라.)
